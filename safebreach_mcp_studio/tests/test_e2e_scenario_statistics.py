...
Covers test-plan items T-31 … T-35 and T-44: scoring a real scenario against a real
fleet, both input forms, the paired role breakdown and its cap, the three-state
simulator model, the cap tally, and simulator scoping.

ZERO MOCKS — all calls hit real SafeBreach APIs.

The tools take only a saved plan's numeric id or a run's planRunId, so the scenario
under test is SAVED for the module: an existing OOB scenario's steps are stored as a
temporary custom plan through the config plans API, scored by id, and deleted on
teardown. T-35 builds its own over-the-cap step the same way. Where a case needs a
console property that cannot be discovered or built (an offline simulator), it SKIPS
naming that precondition rather than passing vacuously.
...
PLAN_STEP_KEYS = ('name', 'attacksFilter', 'attackerFilter', 'targetFilter', 'systemFilter')


# ---------------------------------------------------------------------------
# Fixture helpers — a temporary saved plan, and discovered ids
# ---------------------------------------------------------------------------
...
def _plan_headers(console):
    return {'Content-Type': 'application/json', **get_auth_headers_for_console(console)}


@contextmanager
def _temporary_plan(console, name, steps):
    """Save steps as a custom plan for the duration of the block, then delete it."""
    headers = _plan_headers(console)
    created = requests.post(_plans_url(console), headers=headers, timeout=60,
                            json={'type': 'validate', 'draft': False,
                                  'name': f"{name} {int(time.time())}", 'steps': steps})
    assert created.status_code == 201, f"plan create failed: {created.status_code} {created.text[:300]}"
    plan = created.json()['data']
    try:
        yield plan
    finally:
        deleted = requests.delete(_plans_url(console, plan['id']), headers=headers, timeout=60)
        assert deleted.status_code in (200, 204, 404), (
            f"plan {plan['id']} was not cleaned up: {deleted.status_code} {deleted.text[:200]}")


@pytest.fixture(scope='module')
def saved_scenario():
    """An existing OOB scenario's steps, saved as a temporary custom plan for the module."""
    source = next((s for s in _fetch_all_scenarios(E2E_CONSOLE) or [] if s.get('steps')), None)
    if source is None:
        pytest.skip(f"no OOB scenario with steps on {E2E_CONSOLE} to save and score")
    steps = [{key: step[key] for key in PLAN_STEP_KEYS if key in step} for step in source['steps']]
    with _temporary_plan(E2E_CONSOLE, 'SAF-35508 e2e scenario', steps) as plan:
        logger.info(f"saved '{source.get('name')}' ({len(steps)} steps) as plan {plan['id']}")
        yield {'scenario_id': str(plan['id']), 'name': plan['name'], 'step_count': len(steps)}


def _discover_test_id(console):
    """A completed run's planRunId."""
    listing = sb_get_tests(console=console, status_filter='completed')
    for test in listing.get('tests_in_page', []):
        if test.get('test_id'):
            return str(test['test_id'])
    pytest.skip(f"no completed test on {console} to score by test_id")
...
@pytest.mark.e2e
@skip_e2e
def test_T_31_a_saved_plan_is_scored_against_the_live_fleet(saved_scenario):
    """A saved configuration is scored without being run."""
    result = sb_get_scenario_simulation_counts(
        console=E2E_CONSOLE, scenario_id=saved_scenario['scenario_id'])

    assert result['steps_returned'] == saved_scenario['step_count'], (
        "the console scored a different number of steps than the plan holds")
...
@pytest.mark.e2e
@skip_e2e
def test_T_31_scoring_queues_no_test(saved_scenario):
    """Scoring must not be a run: no answer field names a test, and no test of the plan appears."""
    result = sb_get_scenario_simulation_counts(
        console=E2E_CONSOLE, scenario_id=saved_scenario['scenario_id'])

    for key in ('planRunId', 'test_id', 'testId', 'runId'):
        assert key not in result, f"a read-only scoring answer carried {key}"
    runs = sb_get_tests(console=E2E_CONSOLE, name_filter=saved_scenario['name'])
    assert not runs.get('tests_in_page'), "scoring the plan queued a test of it"


# ---------------------------------------------------------------------------
# T-32 — both input forms against a live console
# ---------------------------------------------------------------------------


@pytest.mark.e2e
@skip_e2e
def test_T_32_both_tools_accept_a_plan_id_and_a_test_id(saved_scenario):
    """The two tools share an input layer, so neither can drift on what it accepts."""
    test_id = _discover_test_id(E2E_CONSOLE)
    print(f"\nT-32 inputs: plan {saved_scenario['scenario_id']}, test {test_id}")

    for tool in (sb_get_scenario_simulation_counts, sb_get_scenario_blocked_entities):
        assert tool(console=E2E_CONSOLE, scenario_id=saved_scenario['scenario_id']) is not None
        assert tool(console=E2E_CONSOLE, test_id=test_id) is not None
...
def test_T_32_an_oob_scenario_uuid_is_refused_without_contacting_the_console():
    ...
    for tool in (sb_get_scenario_simulation_counts, sb_get_scenario_blocked_entities):
        with pytest.raises(ValueError) as excinfo:
            tool(console=E2E_CONSOLE, scenario_id='2f4a1f5e-0000-4000-8000-000000000000')
        assert 'custom plan' in str(excinfo.value) and 'test_id' in str(excinfo.value)


@pytest.mark.e2e
@skip_e2e
def test_T_32_a_scenario_body_is_not_accepted():
    """The ad-hoc body is gone from both tools, not silently ignored."""
    for tool in (sb_get_scenario_simulation_counts, sb_get_scenario_blocked_entities):
        with pytest.raises(TypeError):
            tool(console=E2E_CONSOLE, scenario={'steps': [{'name': 'x'}]})
...
@pytest.mark.e2e
@skip_e2e
def test_T_33_the_breakdown_is_present_exactly_when_the_fleet_is_under_the_cap(saved_scenario):
    ...
    for step in result['steps']:
        ...
        if offered > SIMULATOR_LISTING_CAP:
            ...
            assert 'saved plan' in rendered and 'name simulator_ids' in rendered, (
...
@pytest.mark.e2e
@skip_e2e
def test_T_33_every_offered_simulator_carries_both_role_numbers(saved_scenario):
...
@pytest.mark.e2e
@skip_e2e
def test_T_33_naming_simulator_ids_answers_them_in_both_roles(saved_scenario):
    ...
    if not named:
        first = sb_get_scenario_simulation_counts(console=E2E_CONSOLE, scenario_id=scenario_id)
        named = sorted(_offered_union(first['steps'][0])) if first['steps'] else []
    if not named:
        pytest.skip("no connected simulator on this console to name")

    result = sb_get_scenario_simulation_counts(
        console=E2E_CONSOLE, scenario_id=scenario_id, simulator_ids=','.join(named[:2]))
...
@pytest.mark.e2e
@skip_e2e
def test_T_34_a_switched_off_simulator_is_excluded_not_blocked(saved_scenario):
...
@pytest.mark.e2e
@skip_e2e
def test_T_34_the_verdict_follows_the_counts_not_the_list_lengths(saved_scenario):
...
@pytest.mark.e2e
@skip_e2e
def test_T_34_constraint_meanings_are_relayed_or_declared_absent(saved_scenario):
...
# ---------------------------------------------------------------------------
#
# No shipped scenario blocks more than the cap on an ordinary fleet, so this fixture
# is BUILT: a single step holding every exfiltration attack in the playbook, aimed at
# one simulator the counts tool has measured at zero as a target. Every attack in the
# step is then blocked for a reason the console itself records. Both the probe that
# finds that simulator and the step itself are saved plans, deleted after.
...
@pytest.fixture(scope='module')
def capped_scenario():
    """An over-the-cap step saved as a plan; the plan is deleted after."""
    ...
    with _temporary_plan(E2E_CONSOLE, 'SAF-35508 e2e cap probe', [_step(ids, CONNECTED)]) as probe:
        answers = sb_get_scenario_simulation_counts(
            console=E2E_CONSOLE, scenario_id=str(probe['id']),
    ...
    step = _step(ids, {'simulators': {'operator': 'is', 'values': [target], 'name': 'simulators'}})
    with _temporary_plan(E2E_CONSOLE, 'SAF-35508 e2e cap fixture', [step]) as plan:
        logger.info(f"T-35 fixture: {len(ids)} attacks, target {target}, saved as plan {plan['id']}")
        yield {'ids': ids, 'target': target, 'plan_id': plan['id']}


@pytest.mark.e2e
@skip_e2e
def test_T_35_past_the_attack_cap_a_tally_replaces_the_list(capped_scenario):
    """Summarise-don't-sample, against a real constraint payload."""
    result = sb_get_scenario_blocked_entities(
        console=E2E_CONSOLE, scenario_id=str(capped_scenario['plan_id']))
    step = result['steps'][0]
...
@pytest.mark.e2e
@skip_e2e
def test_T_35_every_capped_attack_still_carries_its_blockers_when_named(capped_scenario):
    """attack_ids is the documented route back to exact reasons — for every capped attack."""
    ids = capped_scenario['ids']
    result = sb_get_scenario_blocked_entities(
        console=E2E_CONSOLE, scenario_id=str(capped_scenario['plan_id']),
        attack_ids=','.join(str(i) for i in ids))
    step = result['steps'][0]
...
def test_T_35_the_all_constraints_payload_is_measured(capped_scenario):
    ...
    payload = _fetch_scenario_statistics(E2E_CONSOLE, {'name': '', 'id': capped_scenario['plan_id']},
...
@pytest.mark.e2e
@skip_e2e
def test_T_44_scoping_to_a_simulator_moves_no_total_and_explains_what_it_lists(saved_scenario):
    ...
    """
    scenario_id = saved_scenario['scenario_id']
    plain = sb_get_scenario_blocked_entities(console=E2E_CONSOLE, scenario_id=scenario_id)
    ...
    scoped = sb_get_scenario_blocked_entities(
        console=E2E_CONSOLE, scenario_id=scenario_id, simulator_ids=simulator_id)
...
@pytest.mark.e2e
@skip_e2e
def test_T_44_an_offline_simulator_is_reported_excluded_with_no_scoped_list(saved_scenario):
