...
automation that drives the real console AI agent (Helm) on the provisioned feature test console, with an
...
**Run:** `pytest … --mgmt_address <feature-test-console>` → **4 passed in 97.5s.**
...
## Provenance — the calls hit OUR migrated tools, not a legacy copy (critical for a migration test)
Because an internal MCP service still exposes the same four tools (SAF-32798 migrates away from
...
1. **mcp-proxy log**: the `[configuration]` server (= `safebreach_mcp_config`,
   host of the 4 tools) logged `ListToolsRequest` + `CallToolRequest` for each test in BOTH run windows
   (17:51–17:53 and 17:57–17:58 UTC). The internal source service is separate; its calls do not appear here.
2. **Output envelope**: the tool outputs in the transcript use the NEW repo snake_case pagination shape
   ...
   `total_integrations`, `applied_filters`) with ZERO occurrences of the legacy camelCase envelope
   shape (a list plus a total-count field). Only the new tools emit those keys.
Conclusion: Helm invoked the migrated safebreach-mcp Config-server tools, not the legacy copies.
