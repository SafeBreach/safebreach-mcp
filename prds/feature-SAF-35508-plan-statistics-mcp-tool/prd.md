...
| **Current Phase** | All six phases implemented. **Feature not signed off**: T-28…T-31, T-40 (automatic e2e) and T-32, T-33, T-35 (manual) have never run — no Validate console environment exists for this ticket. |
...
- **CORRECTED 2026-08-27 — `scenario_id` is a passthrough only for a custom plan, never for an OOB scenario.**
  ...
  **Consequence**: the fetch core keeps its passthrough (an integer id still goes straight to Core, so
...
- [x] It also accepts a `scenario_id`, passed to Core as `{id}` rather than resolved client-side. *(AC-1)*
...
| Phase 5: Public function + tool registration | 🔄 In Progress | - | 1714dba | Code complete, T-24…T-27 green. **T-28…T-31 and T-40 BLOCKED** — no Validate console environment exists for this ticket. Tests authored and collecting; they run as soon as one is provisioned. |
