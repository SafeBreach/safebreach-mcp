...
> `get_scenario_simulation_counts` ("how many simulations, and from which simulators?") and `get_scenario_blocked_entities`
...
|---|---|---|---|
| `get_scenario_simulation_counts` | How many simulations would this scenario produce, and which simulators produce them? | `studio_server.py:1673` | `sb_get_scenario_simulation_counts` — `studio_functions.py:3999` |
...
Per-tool parameters: only `get_scenario_blocked_entities` has one — `attack_ids=None` (optional,
comma-separated). The counts tool has none of its own, so its whole surface is the six shared names.
...
#### The counts answer relays three data fields, and no others (D11, 2026-09-14)

Per step: the **simulation count**, the **attacker simulator map** and the **target simulator map**. The
attack map is **not** carried — enumerating attacks is not this tool's question. Alongside those three sit
`step_index`, `counts_computed` and `is_limit_reached`, which are structural rather than data: they are what
keeps a missing count distinguishable from a zero.

**What this removed.** Phase 13's paginated attack listing, and with it `page` / `page_size`. The counts tool
no longer names any attack. *"Which attacks would run"* as an **enumeration** is therefore answerable by
neither tool — a real reduction in capability, recorded rather than glossed. What remains reachable is the
per-attack question: name an id to `get_scenario_blocked_entities` via `attack_ids` and it answers `ran` with
its count, or `blocked` with the constraint.

**Nothing constraint-derived is built either (D14).** `counts_only=True` is threaded from the counts
tool down to the shaping layer, which then emits **no** attack map, zero-impact list, conflicts or catalog —
**absent, not empty**. Absence cannot be misread as "looked and found nothing", which is the same
null-versus-zero distinction the counts themselves keep. Measured on a field-scale step (2,000 moves, 44
simulators): **26 step keys → 18, 7,007 → 3,924 bytes, 0.96 ms → 0.03 ms** of shaping. It is a per-call flag
rather than a deletion because `moves` is the *sibling's* entire answer — a blocked attack **is**
`moves[id] == 0`, and `_conflict_severity(moves.get(attack_id))` tells blocking from reducing — and because
`plan/statistics` offers no parameter to stop sending it.

**What this bought.** The tool now makes **zero playbook requests, unconditionally.** It carries no attack
ids at all, so there is nothing whose name would need resolving — the property Phase 11 won at a measured
...
- **The simulator maps are never capped, so the counts answer reports true counts.** The 100-entry cap exists
  for `moves`, which a real console returns thousands of ids in — one measured step held 2,000+. A simulator
  map is bounded by the fleet (44 entries on the estate this was measured against), so capping it bought
  nothing and cost the truth: it forced "at least N of M" hedging over a number that is exactly known. The cap
  is aimed, not removed — `attacks` is still capped, and blocked-entities still reports its coverage as a
  lower bound when it bites.
...
**`get_scenario_simulation_counts`** — the answer is the number, which simulators produce it, and the
honesty about whether it is a measurement at all:
...
- **Step 0** — 17 simulations. Coverage: 2 of 7 target simulators, 2 of 7 attacker simulators produce simulations.
  - Contributing attackers: 4488a72d-469f-4ab9-934b-17ff98cdf045 (10), 56211658-2cc6-459f-98ea-d0e163ca9251 (7)
  - Contributing targets: 4488a72d-469f-4ab9-934b-17ff98cdf045 (10), 56211658-2cc6-459f-98ea-d0e163ca9251 (7)
- **Step 1** — 26 simulations. Coverage: 2 of 7 target simulators, 2 of 7 attacker simulators produce simulations.
  - Contributing attackers: 4488a72d-469f-4ab9-934b-17ff98cdf045 (14), 56211658-2cc6-459f-98ea-d0e163ca9251 (12)
  - Contributing targets: 4488a72d-469f-4ab9-934b-17ff98cdf045 (13), 56211658-2cc6-459f-98ea-d0e163ca9251 (13)
...
Three data fields per step and no more (D11): the count, and the two simulator maps. **No attacks appear** —
the coverage clause covers simulators only, and there is no listing. A step producing `0` is reported but not
explained; that is the other tool's question, and the hint routes there by name. The *Contributing* lines are
...
- [x] Three tools are registered, each answering one question and rendering only its own slice:
...
- [x] `get_plan_statistics` is **unregistered** as an MCP tool. `sb_get_plan_statistics` survives unchanged as
...
- [x] Caller-facing vocabulary is **scenario**, not plan: tool names, parameter names, tool descriptions and
...
- [x] CLAUDE.md entry 25 is replaced by three entries, and the rate-limiting gate table is **not** extended. *(D4, AC-12)*
- [x] The registration and e2e tests that named the retired tool (T-24…T-27, T-28…T-31, T-40) are retargeted
...
| Phase 13: The counts tool names which attacks will run, paginated (D8) — **withdrawn by D11** | ✅ Complete, then removed | 2026-09-03 | 335d4b9 |
...
### Phase 10 — `attack_ids` becomes required; the last overlap is removed

**Semantic change**: `get_scenario_attack_blockers` stops answering "what is blocked?" It explains the attacks
the caller names, and nothing else.

**Why this is not a merge.** The owner's question after Phase 9 was whether entry 27 is a subtype of entry 26
and the two should become one. Neither contains the other: `get_scenario_blocked_entities` reports **simulators**,
which the blockers tool never does; and with ids named, the blockers tool is a **total function over those ids** —
`ran` (with its count), `absent`, `not_computed`, `count_map_truncated` — none of which an existential "here is
what is blocked" list can express. An id missing from such a list could have run, could be absent, or could sit in
an unscored step, and collapsing three answers into one guess is the silence-for-an-answer failure this whole
decomposition exists to remove. Merging them would produce a tool whose output shape flips on an optional
parameter: exactly the shape of the retired `get_plan_statistics`.

**What was genuinely duplicated** is the mode nobody named ids in. It listed every fully-blocked attack from
`zero_impact_attacks`, deduped by the same `_blocked_across_steps` rule the verdict already uses — and
`get_scenario_blocked_entities` already counts those distinct attacks in its verdict, already carries each
blocker inline, and already narrows the catalog to the cited codes. The mode added a list where the sibling had
a count, and gave one scoring two owners and two phrasings.

**Implementation details**
- `sb_get_scenario_attack_blockers` rejects a missing or blank `attack_ids` **before scoring**, with a message
  that names `get_scenario_blocked_entities` rather than leaving the caller to find it. The separator-only
  rejection stays, its "omit it entirely" escape hatch replaced by the same redirect.
- The wrapper takes `attack_ids` first so FastMCP marks it **`required` in the input schema**. A runtime
  rejection costs a round trip; the schema is what actually reaches the calling agent.
- `_project_attack_blockers` loses the unnamed branch, `blocked_attacks` and `blocked_attacks_listing_capped`;
  `_listing_may_be_partial` had no other caller and is deleted.
- The narrator renders dispositions only. Its "no fully-blocked attack was found" line is gone with the listing
  it described; the `any_step_scored` guard is **kept and now reachable** — every disposition reading
  `not_computed` is a statement about each id, while "nothing was scored at all" is a fact about the report, and
  without it a caller reads the page as bad luck with the ids they chose.

**Changes**

| File | Description |
|---|---|
| `safebreach_mcp_studio/studio_functions.py` | `attack_ids` required; unnamed listing, its capped flag and `_listing_may_be_partial` deleted |
| `safebreach_mcp_studio/studio_server.py` | `attack_ids` first in the wrapper so the schema requires it; description rewritten; renderer reduced to dispositions |
| `safebreach_mcp_studio/tests/test_studio_functions.py` | Removed-mode tests retargeted to named ids; `_required_kwargs` shared by the suites that exercise all three; schema-required and redirect tests added |
| `safebreach_mcp_studio/tests/test_e2e_plan_statistics.py` | T-31's all-three parametrization supplies the required id |
| `CLAUDE.md` | Entry 27 states the requirement and why the unnamed mode went; §"disjoint by construction" is now true rather than aspirational |

**Git commit**: `refactor(SAF-35508): require attack_ids and remove the last overlap between the three tools`
...
### Phase 12 — Two tools: merge the blockers, filter before capping, enrich both entity kinds

**Semantic change**: three tools become two. `get_scenario_attack_blockers` is retired; the per-named-id
question it answered becomes a **section** of `get_scenario_blocked_entities`, which additionally reports
what each blocked attack and each in-scope simulator actually *is*.

**Why this is not a re-merge of what Phase 10 separated.** Phase 10's objection was to a tool whose **output
shape flipped** on an optional parameter — an existential list without `attack_ids`, per-id dispositions with
it. That is not what this is. Here the sections are **additive**: the report always carries a verdict, a
blocked-attack list and a blocked-simulator list, and when `attack_ids` is supplied it *also* carries one
...
- `attack_ids` becomes **optional again** (the requirement existed to remove an overlap between two tools;
  with one tool there is no overlap). Its meaning changes from *"which question"* to *"narrow this report"*.

#### 12a — Filter by id **before** the caps, not after

Today the filter lives in the projection and reads maps the shaping layer has already capped
(`_index_attack_occurrences`: *"Both maps are capped"*). So the caps bite first:

| a named attack sorting past… | today's answer |
|---|---|
| `COUNT_MAP_CAP` (100) | `count_map_truncated` — *"whether it ran is unknown"* |
| `ZERO_IMPACT_CAP` (50) | reported blocked, but its **blockers are gone**; the narrator says "truncated" |

A caller can therefore name **one** attack and be told "I cannot tell you", purely because a hundred attacks
they never asked about sorted ahead of it.

**Fix: pin, then cap.** `_cap_list` and `_cap_count_map` take the named ids and place their entries first,
filling the remainder of the cap with the rest. This is deliberately *not* a filtered second shaping pass:

- a named attack keeps its entry **and** its full blocker detail, whatever the scenario's size;
- `<map>_total` is untouched, so coverage denominators stay the scenario's true totals;
- **the verdict is still computed over the whole report**, which matters — filtering the verdict would make
  *"nothing is blocked"* silently mean *"none of your two attacks is blocked"* while forty others are. Those
  are different claims and must never share a sentence. The narration states which is which.

**Consequence to record, not hide:** with pinning, `count_map_truncated` becomes **unreachable for a named
id** — if the id is in the response's map at all it is now shown. That is the point of the change. The
disposition stays in the vocabulary (it still describes the truth about a capped map) and T-44's
`test_an_id_past_the_count_map_cap_is_not_reported_absent` flips from expecting `count_map_truncated` to
expecting `blocked`.

#### 12b — Enrich the blocked attacks from their own records

**Cost: zero additional requests.** Since `de8afff` each blocked attack's name comes from
`GET /api/kb/vLatest/moves/{id}` (~10 KB); we take `name` and discard the other 13 fields. One of them is
`content`, and `playbook_types._extract_platform_data` (`:222`) already derives attacker/target platform from
`content.nodes.*.constraints.os` via the `isSource`/`isDestination` role mapping.

Add to each blocked attack: `target_platform`, `attacker_platform`. **Explanatory fields only** — not
description, MITRE or parameters: they do not explain a block, and fifty full records is a response-size
problem.

#### 12c — Return in-scope simulator information

**Scope is in-scope simulators, not blocked ones.** The report that prompted this had **no blocked
simulator** (*"1 of 1 target simulators ... produce simulations"*); the simulator that mattered was the one
that **worked** — it ran the other attack and was simply wrong for the blocked one. Enriching only zero-count
simulators would have missed the case entirely.

Today a blocked simulator prints as a bare UUID: `_zero_impact_simulators` emits
`{simulator_id, blockers:[{code, side, attack_count}]}`, and the `simulator_names` slot that already exists in
`_shape_statistics_step` is **never populated** on this path.

Join one `get_console_simulators` call per report, by id, over the union map's simulators (capped at 100/step,
true total stated), carrying: **name**, **OS type + version**, **`isConnected` / `isEnabled` / approved**,
**roles + assets** (`isInfiltration`, `isExfiltration`, `isAWSAttacker`, …), and its **simulation count in
this step**. Criticality, labels and proxy support are excluded — they explain nothing about a block.

#### The boundary this phase must not cross

Platforms and simulator OS are reported as **adjacent facts**, never as a cause. The report may say *"this
attack declares `target_platform: LINUX`"* and *"the one in-scope target simulator is WINDOWS"*; it must not
say *"blocked because of OS"*. The console cited different codes, and MCP vendors no constraint vocabulary and
no root causes — the same discipline that governs relaying `description` verbatim.

**Changes**

| File | Description |
|---|---|
| `safebreach_mcp_studio/studio_functions.py` | `attack_ids` threaded to the shaping layer; pin-then-cap in `_cap_list`/`_cap_count_map`; attack platform + simulator enrichment; `_project_attack_blockers` folded into `_project_blocked_entities` as a section |
| `safebreach_mcp_studio/studio_server.py` | `get_scenario_attack_blockers` unregistered (15 → 14 tools); merged description and narration; simulator and platform rendering |
| `safebreach_mcp_playbook/playbook_functions.py` | per-move fetch returns the explanatory fields, not only `name` |
| `CLAUDE.md` | entries 26/27 become one; entry 27's overstated "never dropped from the list that exists to explain it" becomes true rather than aspirational |

**Tests written** (four, not the five sketched — the "verdict is unfiltered" and "named id that ran" cases
belong to one property and are pinned together in T-61)
- **T-60** — a named attack survives both per-step caps, entry and explanation alike, including the case only
  the count-map pin can carry: an attack that **ran**, so it is in the counts map and not the zero-impact list.
- **T-61** — a scoped report narrows what is shown, never what is claimed: the verdict stays scenario-wide, a
  named id that ran is answered, the counts map is never filtered, and the source report is not mutated.
- **T-62** — a blocked attack carries its declared platforms and an in-scope simulator its name/OS/state, with
  no causal claim and no implied remedy; a failed lookup costs the identities, not the report.
- **T-63** — the merged tool is registered, `get_scenario_attack_blockers` is gone from both the registry and
  the module, and the tool count is 14.

**Deviation from this spec, on the evidence R18 demanded.** The spec said 12c would be "one
`get_console_simulators` call per report, joined by id". Measured on `saf-35508` before shipping: that call is
**499 KB for four nodes — ~125 KB per node**, which extrapolates to **~61 MB on a 500-node console**: the same
magnitude as the playbook fetch Phase 11 removed, and the reason R18 said measure rather than assume. Two
narrowing routes were probed: `details=false` drops the payload to **1.8 KB** but carries **neither OS nor
connection state**, so it cannot answer the question these details exist for; and the per-node endpoint costs
the same ~125 KB per node but is fetched only for the nodes shown. So the implementation resolves **per id**,
eight at a time, for at most **`SIMULATOR_DETAIL_CAP = 10`** nodes per step, ordered blocked-first, with the
narration stating how many of how many were described.

**A second narrowing came from the live check**, not the plan: a report scoped to one attack was still resolving
every blocked attack's record — three lookups to render one. The attack fill now takes the pinned ids too.

**Measured after, on the live console** (`saf-35508`, 4-step OOB scenario):

| call | bulk KB | per-move | fleet | per-node | total |
|---|---|---|---|---|---|
| `get_scenario_simulation_counts` | 0 | 0 | 0 | 0 | **0.00 MB** |
| `get_scenario_blocked_entities` (whole) | 0 | 3 | **0** | 2 | 0.23 MB |
| `get_scenario_blocked_entities` (`attack_ids`) | 0 | **1** | **0** | 2 | 0.22 MB |

A naive build of the same feature — the bulk playbook plus the fleet listing — would have been ~59 MB.

**Two mutations survived the first test pass, and both mattered.** The count-map pin could be deleted with the
suite green, because every fixture's named id was also in the zero-impact list, which has its own pin; a case
where the named attack **ran** (so it is in the counts map and *not* in the zero-impact list) now covers it.
And the R16 verdict-ordering guard could not be falsified at all — the verdict reads **two independent
unfiltered sources** (`attacks` and `zero_impact_attacks`) and the projection narrows only its own copy, so
scoping either alone cannot change it. That is a stronger design than R16 assumed; what is pinned instead is
the invariant that could actually regress — the counts map is never filtered, and the projection never mutates
the report it was given.

**Git commit**: `refactor(SAF-35508): merge the blockers into blocked-entities, filter before capping, enrich both entity kinds`

---

### Phase 13 — The counts tool names which attacks will run, paginated

> **Withdrawn by D11 (2026-09-14).** Everything this phase added — the paginated attack listing,
> `page`/`page_size`, `_attacks_page`, `_validate_paging` and `_name_counts_pages` — has been removed. The
> counts answer now relays three per-step data fields (simulation count, attacker and target simulator maps)
> and enumerates no attacks; **"which attacks would run" as an enumeration is answered by neither tool**. The
> per-attack question survives on `get_scenario_blocked_entities` via `attack_ids`. T-64/T-65/T-66 are
> tombstoned. The phase is kept below because its measurements — the 58.6 MB listing, the 9,659-id step, the
> cost of naming per page — are what justified the design D11 then simplified, and because the
> zero-playbook-request property it traded away is now unconditional. Read it as history, not as shipped
> behaviour.

**Semantic change**: `get_scenario_simulation_counts` answers *"which attacks will run, and how many
simulations"* rather than *"how many simulations"*. It gains no constraint data — explaining a zero stays the
other tool's question.

**Why pagination is not optional here.** A step's attack map holds up to **9,659** entries (measured live on
`saf-35508`) and is display-capped at 100. So "which attacks will run" is **not answerable in full today at
any price** — the cap makes it a sample presented as a list. Naming attacks without paging would either keep
that sample or resolve names for an unbounded set; above ~100 ids the per-id path loses to the bulk call and
we would fall back to the **58.6 MB** listing, undoing Phase 11 on the one tool that currently costs zero
playbook requests.

**Implementation details**
- `page` (default 0) and `page_size` (default 10, matching this repo's `PAGE_SIZE` convention), applied to the
  step's attack map ordered deterministically; the step's **true total** is always stated, so a page is never
  mistaken for the whole.
- Names resolved for the **page only** — ~10 ids ≈ 0.08 MB and ~0.8 s, versus 58.6 MB.
- **Stated trade-off:** this costs the zero-playbook-request property Phase 11 won for this tool. That is a
  conscious exchange — the tool's purpose is now to name attacks — and it is bounded to one page, not
  unbounded. A caller who wants the old cost sets `page_size=0`.
- Still no constraints, no zero-impact lists, no catalog: a step producing 0 is reported, never explained, and
  the hint keeps routing to the merged tool.

**Changes**

| File | Description |
|---|---|
| `safebreach_mcp_studio/studio_functions.py` | paginated attack listing in `_project_simulation_counts`; page-scoped name resolution |
| `safebreach_mcp_studio/studio_server.py` | `page`/`page_size` parameters; the listing rendered with its true total |
| `CLAUDE.md` | entry 25 restated: which attacks and how many, paginated |

**Tests required**
- **T-65** — a step with more attacks than `page_size` lists exactly one page and states the true total.
- **T-66** — names are resolved for the page only: a 400-attack step triggers one lookup of 10 ids, never the bulk listing.
- **T-66** — `page_size=0` lists no attacks and resolves no names, preserving Phase 11's property on request.

**Renumbered**: the spec sketched T-65…T-67; Phase 12 consumed through T-63, so these landed as **T-64…T-66**.

**Found live, not by the tests**: the counts tool's routing hint still named `get_scenario_attack_blockers`,
retired one phase earlier — an agent following it would have called a tool that does not exist. The hint is
fixed, and a test now walks every `get_scenario_*` name appearing in either narration and asserts it is
actually registered, so a retirement cannot leave a broken link in agent-facing text again.

**Measured after, on the live console** (`saf-35508`, 4-step OOB scenario, 2 attacks in the largest step):

| call | bulk KB | per-move | total |
|---|---|---|---|
| default page | 0 | 5 | 0.03 MB |
| `page_size=0` | 0 | **0** | **0.00 MB** |

The default costs one lookup per listed attack and nothing else; `page_size=0` is the documented way back to
the zero-request behaviour Phase 11 won.

**Git commit**: `feat(SAF-35508): name which attacks will run, paginated`
...
**Implementation details**
- `_attack_facts` keeps `tags` and `advanced_actions` (`{id: name}` from the `Advanced_Actions` tag).
  ~900 bytes of a 5.9 KB record we already pay for and were discarding.
- **Tags use the repo's existing shape.** The first cut introduced a second one (`{name: [values]}`) alongside
  the shipped `_transform_tags` (`["Attack Type:Malware Pre-Execution", …]`) that
  `get_playbook_attack_details` already returns — two spellings of the same source data, the class of
  duplication PR #91 flagged twice. Caught by the owner asking whether the attacks tool already returned tags.
  `_index_tags` and `_tag_values` are deleted; `_attack_facts` calls `_transform_tags`.
- **That shape drops the ids**, which is exactly why `advanced_actions` is a separate field. Nothing else in
  this repo carries the id — not `get_playbook_attacks`, not `get_playbook_attack_details`, not the tag line —
  so without it `required: [0]` is unresolvable anywhere. This settled the open question of whether
  `_named_advanced_actions` could be dropped in favour of relaying tags alone: it cannot.
...
| **R15** | **Merging re-opens the failure the whole ticket exists to fix.** `get_plan_statistics` was retired for answering three questions at once and leaving the caller to read past two of them. A merged blocked-entities tool that grows a per-id section, platform data and simulator records could drift back into the same shape. | **Medium** | The merged tool's question stays exactly one — *"what will not run, and why"* — and the named-id section is a **filter on that question**, not a second question. The verdict stays one sentence. The line to hold: the moment it starts reporting what *did* run beyond the ids the caller named, it has become two tools again. Phase 13 keeps "what will run" in the other tool precisely so this line is visible. |
| **R16** | **Filtering is a way to hide.** A report scoped to two named attacks can truthfully say "nothing blocked" while forty other attacks in the same scenario are blocked. | **High** | The verdict is computed over the **unfiltered** report and the narration says which scope each claim belongs to. Pinning (12a) narrows what is *shown*, never what is *claimed*. T-62 pins this directly. |
| **R17** | **Adjacent facts read as causation.** Printing "attack declares LINUX" beside "simulator is WINDOWS" invites both the agent and the reader to write "blocked because of OS" — which the console did not say; it cited two different codes. | **Medium** | The same rule that governs constraint descriptions: relay, never author. The narration states the facts adjacently and asserts no cause; T-63 pins the absence of a causal claim. The real-world miss that prompted this (SAF-35508 field feedback, the Akira_v2 case) was an **agent** asserting a remedy the tool never vouched for, so the hint must also say that a remedy is not implied. |
| **R18** | **The simulator join is unmeasured.** One `get_console_simulators` call per report is assumed cheap, but the only console available (`saf-35508`, 4 simulators) is stopped and a large customer fleet was never measured — the exact shape of the playbook mistake Phase 11 fixed. | **Medium** | Measure before shipping 12c: resume the env or use any console with a real fleet, and record the size in the phase notes. If the fleet response is large, narrow the join to the ids in the union map (≤100/step) rather than fetching the whole fleet. Do not ship this on an assumption — that is precisely how the 58.6 MB fetch survived unnoticed. |
...
- **What was built**: **two read-only MCP tools**, each answering exactly one question over the orchestrator's
  `plan/statistics` endpoint — `get_scenario_simulation_counts` ("how many simulations would this scenario
  produce, and which simulators produce them?") and `get_scenario_blocked_entities` ("what will not run, and why?", optionally
...
|------|-------------------|
| 2026-09-14 (i) | **D14 follow-through — `moves` is dropped at the fetch, not merely left unread.** The owner restated the ask after the counts path stopped *shaping* `moves`: for this tool, remove it. One place still held it — `_normalize_step` retained every map field on every path, so the largest field in the response (≈2,000 ids against 44 simulators) sat in memory on a call that never reads it. `fetch_plan_statistics` gains `keep_moves`, which the counts path sets False; the endpoint still sends it, since `plan/statistics` offers no parameter to suppress it, but it is discarded at the earliest point this code controls. Carrying a field nobody reads is how it ends up read by accident. A test asserts `moves` is absent from the normalized step under the flag **and** that the three simulator maps it shares that loop with are untouched. 2180 offline passed; **e2e 12 passed live**. |
| 2026-09-14 (h) | **D14 — the counts path builds nothing it will not report.** Owner: "`get_scenario_simulation_counts` doesn't need zero_impact_*, conflicts, catalog as well". Correct, and it went further than the projection: those were still **constructed** in the shaping layer and dropped three layers later. `counts_only` now short-circuits the shaping step before any constraint-derived key exists, and the report builder skips the catalog rather than assembling an empty one. **Keys are absent, not empty** — an empty list reads as "looked and found nothing", which is exactly the confusion this feature exists to prevent. Measured at field scale (2,000 moves, 44 simulators): **26 step keys → 18, 7,007 → 3,924 bytes, 0.96 ms → 0.03 ms**; the constraint fixture in that measurement did not take, so the real gap on a constraints-bearing response is **larger, not smaller** — stated rather than rounded up. **One premise corrected before acting**: the owner's preceding message said `moves` "doesn't used at all" and should be removed at the fetch stage. It is not unused — it is the *sibling* tool's entire answer (`_zero_impact_attacks(step['moves'], …)`, `_conflict_severity(moves.get(id))`) — and `plan/statistics` has no parameter to suppress it. So this ships as a per-call flag, with a test asserting the sibling still sees all 2,000 attacks and all 1,000 zeros from the identical response, so the flag cannot quietly become a deletion. 2179 offline passed; **e2e 12 passed live**. |
| 2026-09-14 (g) | **D13 — the counts answer reports true simulator counts; the cap is aimed at `moves`.** A real step from the field settled it: `moves` carried **2,000+** ids while `targetSimulators` and `attackerSimulators` carried **44**. The 100-entry cap was written for the former and was silently applying to the latter, so a fleet of 150 would have reported "at least 100 of 150" over a number known exactly. The simulator maps (and the contributing/not-computed splits over them) are now never capped; `attacks` still is, which is what the cap was for. Consequence beyond the counts tool: blocked-entities' simulator coverage is exact too. **T-57 had to move rather than be edited** — D11 had retargeted its capped fixture onto the *simulator* map, which no longer caps, so the invariant is re-anchored on blocked-entities' attack map, the only capped map left; a new class owns the counts side and asserts the opposite property on a 150-simulator fleet. **A stale catalog test surfaced**: `test_each_entry_names_the_runnable_default` still split CLAUDE.md on the **first mention** of a tool name — the exact fragility its siblings were migrated off after it once asserted against the wrong paragraph — so it was reading entry 25's cross-reference instead of entry 26. Re-anchored on the numbered entry, and entry 26 now states its runnable-only semantics in its own words rather than by reference. 2176 offline passed; **e2e 12 passed live**. |
