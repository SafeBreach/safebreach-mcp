...
| R15 | Full parameter pass-through on the scenario tools, **less the two transport settings** — `limit` and `use_cache` are set internally and exposed by neither tool (D10); only defaults differ among the rest, and only where the question differs — `get_scenario_simulation_counts` defaults `get_constraints=False` | T-26, T-27, T-46, T-50, T-56 | Covered |
...
- Expected: The invariant is one fetch per scoring pass, with the projection adding none. A single-pass call produces exactly one statistics call, and `both_counts=True` produces exactly two — never three — because that parameter's defined behaviour is two scoring passes (T-27). Every explicitly-set parameter reaches the call unchanged, including `conflict_detail`, which shapes the report without reaching the wire. The two transport settings are the exception and are asserted the other way round (D10): neither tool exposes `limit` or `use_cache`, and the wire must carry the **internal** value for each — asserting the value sent, rather than merely that the parameters vanished, is what would catch dropping them from the request altogether and handing the orchestrator its own defaults instead of ours. On defaults, the counts function requests no constraints while the other does, and both request runnable counts. No function issues an extra fetch to fill a gap in its projection under any input.
