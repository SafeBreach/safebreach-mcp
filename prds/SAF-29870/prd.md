...
- [~] **Guardrails (NFR)**: caps (≤50 attacks, ≤20 tags) + rate limit implemented + unit-tested; a bulk op cannot crash the console/Helm — **live verification pending**. *(Phase H done / Phase I verify)*
...
| 2026-07-16 | Phase G + H implemented (TDD): `get_playbook_attack_tags` (req 3) + `bulk_add`/`bulk_remove`/`bulk_rename` (req 4, all 3 modes) via configuration bulk endpoints, with guardrail caps (≤50 attacks, ≤20 tags) + rate limiting + partial-failure reporting. 33 new tests; full suite 767 green. Live bulk-safety + Helm-approval + product review remain (Phase I). |
