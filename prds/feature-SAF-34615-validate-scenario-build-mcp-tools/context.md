...
**Status**: `Phase 10: Complete` (Phase 9 skipped — Story, not Bug)

**Draft PR**: [safebreach-mcp#94](https://github.com/SafeBreach/safebreach-mcp/pull/94), stacked on
[#91](https://github.com/SafeBreach/safebreach-mcp/pull/91) (SAF-35508, still open/unmerged) — this branch's
tools are not independently mergeable until that lands.
...
| FR14 | **Present simulation counts to the user.** *(Added post-brainstorm, user direction.)* FR6 sources the count data; FR8's "final summary" doesn't itself call out the number. Helm must surface the resulting simulation count(s) — per-step and/or total, in plain language — at two points: **immediately after simulator selection is confirmed for a step** (closing the loop FR5 opens), and **again in the final pre-save summary** (operationalizing FR8/DoD1 — "fully configured" should mean the user has seen what it will do, not just that it was computed). Sourced from `get_scenario_simulation_counts` (§3, SAF-35508 D4). Presentation is skill content (breach-genie), not a tool change — see that PRD's §4. | SAF-34615 (skill) |
